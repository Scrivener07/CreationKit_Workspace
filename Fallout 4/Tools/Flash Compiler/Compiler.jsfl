...
function execute()
{
	...
	*/
	...
	fl.trace(FLfile.read(dirURI+"Flash.Publish.log.txt"));
	if (error)
	{
...
function compile(p_fileURI, p_profile, p_fileName)
{
	...
	doc.publish();
	appendToLog("");
	fl.outputPanel.save(dirURI+"Flash.Publish.log.txt", true);
	fl.outputPanel.clear();
...
{
	log += "\n"+p_str;
	FLfile.write(dirURI+"Flash.Publish.log.txt", "\n"+p_str, "append");
}
...
{
	log = p_str;
	FLfile.write(dirURI+"Flash.Publish.log.txt", p_str);
}
